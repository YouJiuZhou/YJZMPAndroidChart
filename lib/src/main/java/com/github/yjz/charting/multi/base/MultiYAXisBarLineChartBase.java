...
//        canvas.restoreToCount(clipRestoreCount);
    ...
    private float yAxisCalculateOffsets(float offsetLeft) {
        ...
        float baseOffset = Utils.convertDpToPixel(20f);
...
//                    result += (yAxis.getRequiredWidthSpace(renderer.getPaintAxisLabels()) + baseOffset);
                    ...
                    yAxis.setNextYAxisDistance(yAxis.getRequiredWidthSpace(renderer.getPaintAxisLabels()) + baseOffset);

                    if (yAxis.isDrawScale()){
                        yAxis.setNextYAxisDistance(yAxis.getNextYAxisDistance() + AxisRenderer.SCALE_LINE_MAX_LEN);
                    }
            ...
            if (mXAxis.isEnabled()){
                ...
                if (mXAxis.isDrawScale()){
                    ...
                    if (mXAxis.getPosition() == XAxis.XAxisPosition.BOTTOM) {
                        ...
                        offsetBottom += AxisRenderer.SCALE_LINE_MAX_LEN;

                    } else if (mXAxis.getPosition() == XAxis.XAxisPosition.TOP) {

                        offsetTop += AxisRenderer.SCALE_LINE_MAX_LEN;
