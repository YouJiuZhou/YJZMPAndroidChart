...
public class MultiYAxisRenderer extends AxisRenderer {
    ...
    public void renderAxisLabels(Canvas c) {
        ...
        mYAxisNameUnitPaint.setTextSize(mYAxis.getTextSize());
        ...
        if (mYAxis.isAxisDependencyLeft()) {
            ...
            if (labelPosition == YAxis.YAxisLabelPosition.OUTSIDE_CHART) {
                ...
                if (mYAxis.isDrawScale()){
                    xPos = mViewPortHandler.offsetLeft() - xoffset - mYAxis.getYInXAxisOffset()- SCALE_LINE_MAX_LEN;
                }else{
...
//                xPos = mViewPortHandler.offsetLeft() - xoffset - mYAxis.getYInXAxisOffset();
    ...
    protected void drawYAxisNameUnit(Canvas canvas, float offset) {
        ...
        String demoTxt = unitTxt;

        if (TextUtils.isEmpty(demoTxt)) {
            demoTxt = "A";
        }
    ...
    protected void drawScale(Canvas canvas, float startY, float offset) {
        ...
        if (mYAxis.isAxisDependencyLeft()) { //Y轴位置在左边时
            for (int i = 0; i <= 5; i++) {
                ...
                if (i % 5 == 0) {
                    canvas.drawLine(leftX, startY, leftX - SCALE_LINE_MAX_LEN, startY, mAxisScaleLinePaint);//画长刻度线
                } else {
                    canvas.drawLine(leftX, startY, leftX - SCALE_LINE_MIN_LEN, startY, mAxisScaleLinePaint);//画短刻度线
                }
                canvas.restore();
            }
        }
        if (mYAxis.getAxisDependency() == YAxis.AxisDependency.RIGHT) { //Y轴位置在右边时
            for (int i = 0; i <= 5; i++) {
                ...
                canvas.translate(0, offset * i);
                if (i % 5 == 0) {
                    canvas.drawLine(rightX, startY, rightX - SCALE_LINE_MAX_LEN, startY, mAxisScaleLinePaint);//画长刻度线
                } else {
                    canvas.drawLine(rightX, startY, rightX - SCALE_LINE_MIN_LEN, startY, mAxisScaleLinePaint);//画短刻度线
                }
                canvas.restore();
            }
        }
